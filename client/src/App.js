...
function PrimaryMajorMenu() {
  ...
  const sendSelectedPrimaryMajor = async (major) => {
    try {
      const response = await fetch('http://localhost:5000/selected-primary-major', {
        method: 'POST',
        headers: {
  ...
  return (
    <div>
      <select value={selectedPrimaryMajor} onChange={handleSelectChange}>
      ...
      <p>Selected Major: {selectedPrimaryMajor}</p>
    </div>
  );
}
...
function SecondaryMenu(){
  ...
  return (
    <div>
        <select value={selectedSecondary} onChange={handleSelectChange}>
        <option value="Please select an option first!">If applicable, choose a second major or minor</option>
        <option value="Accounting">Accounting</option>
        <option value="Afro-American Studies">Afro-American Studies</option>
        <option value="Animal Science, Animal Biotechnology, and Biomedical Sciences">Animal Science, Animal Biotechnology, and Biomedical Sciences</option>
        <option value="Anthropology">Anthropology</option>
      ...
      <p>Selected Major/Minor: {selectedSecondary}</p>
    </div>
  );
}
